...
export default function GameOverScreen({
        ...
        return (
            <div className="w-full min-w-0 flex flex-col items-center justify-center py-12 relative overflow-x-hidden overflow-y-auto">
                ...
                <div className="text-center mb-12 relative z-10 w-full max-w-4xl min-w-0 px-2">
                    <h1 className="text-emerald-400 text-4xl sm:text-6xl md:text-8xl font-black tracking-tighter mb-4 font-mono glitch-effect text-shadow-glow break-words">
                ...
                {/* ASCII Art Container */}
                <div className="mb-12 border border-emerald-500/20 bg-black/40 p-4 md:p-8 rounded-sm relative group overflow-hidden w-full max-w-4xl min-w-0 mx-auto shadow-[0_0_30px_rgba(16,185,129,0.1)]">
                    ...
                    <pre className="font-mono text-emerald-400 text-[10px] sm:text-xs md:text-sm lg:text-base leading-tight whitespace-pre overflow-x-hidden overflow-y-hidden text-center text-shadow-glow max-w-full">
                        {`     _    __   _         __                               
    ...
    |___/  /_/   \___/\__/  \____/ /_/     \__, /         
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 w-full max-w-2xl min-w-0 mb-12 font-mono text-sm relative z-10 px-4 md:px-0 mx-auto">
                    <div className="bg-black/60 border border-emerald-500/30 p-4 relative group hover:border-emerald-400 transition-colors min-w-0 overflow-hidden">
                        <div className="text-emerald-500/50 mb-1">TARGET</div>
                        <div className="text-emerald-400 text-lg uppercase break-words">{targetWord || 'UNKNOWN'}</div>
                    ...
                    <div className="bg-black/60 border border-emerald-500/30 p-4 relative group hover:border-emerald-400 transition-colors min-w-0 overflow-hidden">
                        ...
                        <div className="text-emerald-400 text-lg uppercase break-words">{reasonMessage || 'SUCCESS'}</div>
    ...
    return (
        <div className="w-full min-w-0 flex flex-col items-center justify-center py-12 relative overflow-x-hidden overflow-y-auto">
            ...
            <div className="text-center mb-8 relative z-10 w-full min-w-0 px-2">
                <h1 className="text-[#FF3366] text-4xl sm:text-6xl md:text-8xl font-black tracking-tighter mb-4 font-mono glitch-effect text-shadow-glow break-words">
                    {defeatTitle}
                </h1>
                <p className="text-[#FF3366] text-lg sm:text-xl tracking-widest font-mono uppercase bg-[#1A0B1C]/80 inline-block px-4 py-2 border border-[#FF3366]/30 break-words">
                    {isAiWins ? 'Target Acquired by AI' : 'Connection Terminated'}
                </p>
            </div>

            {/* ASCII Art Container */}
            <div className="mb-8 border border-[#FF3366]/20 bg-[#1A0B1C]/80 p-4 md:p-8 rounded-sm relative group overflow-hidden w-full max-w-4xl min-w-0 mx-auto shadow-[0_0_30px_rgba(255,51,102,0.1)]">
                <pre className="ascii-art text-[#FF3366] font-mono text-[10px] sm:text-xs md:text-sm lg:text-base leading-tight whitespace-pre overflow-x-hidden overflow-y-hidden text-center text-shadow-glow max-w-full">
                    {`      :::    :::   :::::::   :::::::::  :::::::::: :::::::: 
     :+:   :+:   :+:   :+:  :+:    :+: :+:       :+:    :+: 
    +:+  +:+    +:+   +:+  +:+    +:+ +:+       +:+         
   +#++:++     +#+   +:+  +#++:++#+  +#++:++#  +#++:++#++   
...
###    ###   #######   ###        ########## ########       `}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl w-full min-w-0 mb-12 font-mono text-sm relative z-10 mx-auto px-4 md:px-0">
                <div className="bg-[#1A0B1C] border border-[#FF3366]/30 p-4 relative group hover:border-[#FF3366] transition-colors min-w-0 overflow-hidden">
                    <div className="text-[#FF3366]/50 mb-1">TARGET</div>
                    <div className="text-[#FF3366] text-xl uppercase break-words">{targetWord || 'UNKNOWN'}</div>
                ...
                <div className="bg-[#1A0B1C] border border-[#FF3366]/30 p-4 relative group hover:border-[#FF3366] transition-colors min-w-0 overflow-hidden">
                    ...
                    <div className="text-[#FF3366] text-xl uppercase break-words">{reasonMessage || 'TABOO WORD DETECTED'}</div>
