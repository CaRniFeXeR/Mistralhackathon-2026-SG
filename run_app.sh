...
#    ./run_app.sh --mode dev   # backend on :8009 + Vite HMR on :5173
#    ./run_app.sh --mode prod  # build frontend, serve everything on :8009
#
# ─────────────────────────────────────────────────────────────
set -e
...
STALE=$(lsof -ti:8009 2>/dev/null || true)
if [[ -n "$STALE" ]]; then
    echo "⚠️  Killing stale process on :8009 (PID $STALE)..."
...
if [[ "$MODE" == "dev" ]]; then
    ...
    python3 -m uvicorn backend.app.main:app --host 0.0.0.0 --port 8009 --reload --log-level info >> "$DIR/backend.log" 2>&1 &
    BACKEND_PID=$!
...
#  PROD MODE
#  • Build frontend
#  • Serve everything from FastAPI on :8009
# ────────────────────────────────────────────────────────────
elif [[ "$MODE" == "prod" ]]; then
    echo ""
    echo "╔══════════════════════════════════════════════╗"
    ...
    echo ""
    echo "╔══════════════════════════════════════════════╗"
    echo "║  Serving on  →  http://localhost:8009        ║"
    echo "╚══════════════════════════════════════════════╝"
    echo ""
    ...
    python3 -m uvicorn backend.app.main:app --host 0.0.0.0 --port 8009 --log-level info >> "$DIR/backend.log" 2>&1 &
    BACKEND_PID=$!
